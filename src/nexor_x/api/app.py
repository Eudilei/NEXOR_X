...
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px Segoe UI,Arial,sans-serif}.top{height:64px;display:flex;align-items:center;padding:0 24px;border-bottom:1px solid var(--line);background:#0c121d}.brand{font-size:21px;font-weight:800;letter-spacing:1.5px}.brand span{color:var(--accent)}.mode{margin-left:auto;padding:7px 12px;border:1px solid #24553f;border-radius:8px;color:var(--ok);background:#10231b}.layout{display:grid;grid-template-columns:220px 1fr;min-height:calc(100vh - 64px)}aside{border-right:1px solid var(--line);padding:18px 12px;background:#0b111b}.nav{padding:12px;border-radius:8px;color:var(--muted);margin:4px}.nav.active{background:#172236;color:var(--text)}main{padding:22px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.card{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:16px}.label{color:var(--muted);font-size:12px;text-transform:uppercase}.value{font-size:23px;font-weight:750;margin-top:8px}.small{font-size:13px;color:var(--muted);margin-top:8px}.wide{grid-column:span 2}.services{margin-top:14px;display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.service{background:var(--panel2);border:1px solid var(--line);padding:10px;border-radius:8px}.dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:7px;background:var(--bad)}.dot.ok{background:var(--ok)}.dot.warn{background:var(--warn)}textarea{width:100%;height:92px;background:#0b111c;color:var(--text);border:1px solid var(--line);border-radius:8px;padding:10px}button{margin-top:8px;background:var(--accent);border:0;color:#06101e;font-weight:700;padding:10px 16px;border-radius:8px;cursor:pointer}pre{white-space:pre-wrap;color:#cad7e9;min-height:82px}.foot{margin-top:16px;color:var(--muted);font-size:12px}@media(max-width:900px){.layout{grid-template-columns:1fr}aside{display:none}.grid{grid-template-columns:1fr}.wide{grid-column:span 1}.services{grid-template-columns:1fr}}
</style></head><body><header class="top"><div class="brand">NEXOR <span>X</span></div><div class="mode" id="mode">PAPER</div></header><div class="layout"><aside><div class="nav active">Visao Geral</div><div class="nav">Mercado</div><div class="nav">Estrategias</div><div class="nav">Carteira</div><div class="nav">Laboratorio</div><div class="nav">IA</div><div class="nav">Configuracoes</div><div class="nav">Sistema</div></aside><main><div class="grid"><section class="card"><div class="label">Sistema</div><div class="value" id="system">Carregando</div></section><section class="card"><div class="label">Servicos</div><div class="value" id="count">-</div></section><section class="card"><div class="label">BTCUSDT</div><div class="value" id="btc">-</div><div class="small" id="source">Aguardando mercado</div></section><section class="card"><div class="label">Regime</div><div class="value" id="regime">-</div><div class="small" id="marketReason">-</div></section><section class="card"><div class="label">Direcao</div><div class="value" id="direction">-</div></section><section class="card"><div class="label">Confianca do classificador</div><div class="value" id="confidence">-</div><div class="small">Nao representa probabilidade de lucro.</div></section><section class="card"><div class="label">Volatilidade</div><div class="value" id="volatility">-</div></section><section class="card"><div class="label">Dados</div><div class="value" id="freshness">-</div></section><section class="card"><div class="label">Quant Brain</div><div class="value" id="edgeDecision">-</div><div class="small" id="edgeNote">Aguardando evidencias</div></section><section class="card"><div class="label">Edge bruto</div><div class="value" id="rawEdge">-</div><div class="small">Nao calibrado; nao e probabilidade de lucro.</div></section><section class="card wide"><div class="label">Saude dos modulos</div><div class="services" id="services"></div></section><section class="card wide"><div class="label">IA local (Ollama)</div><textarea id="question" placeholder="Qual e o estado atual do sistema?"></textarea><button onclick="ask()">Perguntar</button><pre id="answer"></pre></section></div><div class="foot">NEXOR X 0.5.0 — Evidence Engine e Quant Brain inicial; PAPER obrigatorio e LIVE bloqueado.</div></main></div><script>
...
connect();market();quant();setInterval(()=>{market();quant()},15000);
</script></body></html>"""
